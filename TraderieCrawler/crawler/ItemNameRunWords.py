...
class ItemNameRunWords:
    
    def __init__(self) -> None:
        print('start ItemNameRunWords')
        # uniqueItems 파일
        unique_dia = pd.read_json("jsons/traderie_api_runewords.json")

        # korName을 가지고 있는 데이터 불러오기 (예시)
        tra_kor = pd.read_json("jsons/traderie_runwordskor.json")
        ...
        # tags에서 Item Type 추출
        # unique_dia["type"] = unique_dia["tags"].apply(
        #     lambda tags: [tag["tag"] for tag in tags if tag.get("category") == "Item Type"] if isinstance(tags, list) else []
        # )
        # ✅ korName 공백 제거
        merged_df['korName'] = merged_df['korName'].fillna('').apply(lambda x: re.sub(r'\s+', '', x.strip()))
        # 최종 결과 컬럼 선택 (korName 포함)
        result_df = merged_df[['id', 'name','img','type','sockets', 'korName', 'description_filtered','min-max-filter']]

        print('end ItemNameRunWords')
        # 저장
        result_df.to_json('jsons/runWordsItemList.json',
                          orient='records', force_ascii=False, indent=2)

    # 해당 키워드가 있으면 옵션을 제외시킵니다.
    excluded_keywords = ['Per Char Level'
          , 'Based On Char Level', 'Based On Character Level'
          ,'sec Duration'
          ,'sec. Duration'
          ,'H damage'
          ,'Duration'
